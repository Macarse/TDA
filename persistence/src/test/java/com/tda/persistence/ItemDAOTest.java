...
public class ItemDAOTest {
	...
	public void testItemDAOSave() {
		assertEquals(itemDAO.findAll().size(), testItems.size());
	}

	@Test
	public void testItemDAODelete() {
		itemDAO.delete(testItems.get(itemIndexForTest));
		assertEquals(itemDAO.findAll().size(), testItems.size() - 1);
	}
	...
	public void testItemDAOUpdate() {
		...
		itemDAO.update(testItems.get(itemIndexForTest));
		assertEquals(itemDAO.findById(testItems.get(itemIndexForTest).getId())
				.getName(), updateStringForTest);
		testItems.get(itemIndexForTest).setName(originalName);
	}

	@Test
	public void testItemDAOFindById() {
		Item testItem = itemDAO.findById(testItems.get(itemIndexForTest)
				.getId());
		assertEquals(testItem.getName(), testItems.get(itemIndexForTest)
				.getName());
	}
	...
	public void testItemDAODeleteById() {
		itemDAO.deleteById(testItems.get(itemIndexForTest).getId());
		assertEquals(itemDAO.findAll().size(), testItems.size() - 1);
	}

	@Test
	public void testItemDAOCount() {
		assertEquals(itemDAO.count(), testItems.size());
	}
	...
	public void testItemDAOFindByExample() {
		...
		assertEquals(itemDAO.findByExample(testItem).size(), correctSize);
	}

	@Test
	public void testItemDAOFindByName() {
		List<Item> items = itemDAO.findByName(nameStringForTest);
		assertEquals(items.size(), testItems.size() - 1);
	}

	@Test
	public void testItemDAOFindByDescription() {
		List<Item> items = itemDAO.findByDescription(descriptionStringForTest);
	...
	public void testItemDAOFindByQuantityRange() {
		List<Item> items = itemDAO
				.findByQuantityRange(minQForTest, maxQForTest);
		assertEquals(items.size(), testItems.size() - 1);
	}
}
