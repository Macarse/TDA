...
public class PrevalentDiagnosticForDestinationReportLayout {
	...
	public DynamicReport buildReportLayout() throws ColumnBuilderException,
		...
		// Hacinamiento:
		AbstractColumn columnDiag = ColumnBuilder.getNew()
				.setColumnProperty("diagnostic", String.class.getName())
				.setTitle("Diagnostico").setWidth(new Integer(85)).build();
		...
		drb.addColumn(columnDiag);
		drb.addColumn(columnQuantity);

		drb.setTitle("Diagnosticos Prevalentes por Destino")
		...
		DJChart djChart = new DJPie3DChartBuilder()
				...
				// dataset
				.setKey((PropertyColumn) columnDiag).addSerie(columnQuantity)
				// plot
