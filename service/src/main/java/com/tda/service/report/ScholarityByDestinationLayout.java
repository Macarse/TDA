...
public class ScholarityByDestinationLayout {
	...
	public DynamicReport buildReportLayout() throws ColumnBuilderException,
		...
		// Hacinamiento:
		AbstractColumn columnScholarity = ColumnBuilder.getNew()
				.setColumnProperty("scholarity", String.class.getName())
				.setTitle("Escolaridad").setWidth(new Integer(85)).build();
		...
		drb.addColumn(columnScholarity);
		drb.addColumn(columnQuantity);

		drb.setTitle("Nivel de escolaridad por destino")
		...
		DJChart djChart = new DJPie3DChartBuilder()
				...
				// dataset
				.setKey((PropertyColumn) columnScholarity).addSerie(columnQuantity)
				// plot
